...
| Key file lost **and** password manager copy lost | Transcripts are gone. Tickets, audit, users, money are unaffected. Provision a fresh `k1` (section 2); the old conversations can no longer be read, so remove them with `DELETE FROM assistant_conversations;` (messages and proposals go with them). Setting `assistant_retention_days=0` does NOT purge: 0 or less means "keep forever". |
